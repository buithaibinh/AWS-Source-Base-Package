...
class HomePage : AppCompatActivity(), View.OnClickListener {
    ...
    override fun onStart() {
        ...
        Log.d("HHAHAA", subscriptionListUser.toString())
    }
    ...
    private fun getListUser() {
        App.awsAppSyncClient.query(AllUsersQuery.builder().build())
    ...
    override fun onClick(view: View) {
        when (view.id) {
            ...
            R.id.sendMessage -> {
                NetworkClient.service.sendHello(mapOf(
                    "message" to "Hello",
                    "userInfo" to mapOf(
            ...
            R.id.sendMessageAll -> {
                NetworkClient.service.sendHelloAll(
                    mapOf(
                        "message" to "Hello"
                    )
                ).onSuccess {
            ...
            R.id.btnReport -> {
                App.pinpointManager.sessionClient.startSession()
                logEvent()
            }
        }
    ...
    private fun logEvent() {
        val event = App.pinpointManager.analyticsClient.createEvent("EventName")
            .withAttribute("Report", "ByNamAnh")
            .withMetric("Count", Math.random())
        App.pinpointManager.analyticsClient.recordEvent(event)
        App.pinpointManager.analyticsClient.submitEvents()
    ...
    private fun deleteUser(userName: String) {
        val deleteUser = DeleteUserMutation.builder()
            .userName(userName)
            .build()
        App.awsAppSyncClient.mutate(deleteUser)?.enqueue(deleteUserCallback)
    }
...
//            }
    ...
    private fun subscriptionListUser() {
        ...
        subscriptionListUser = App.awsAppSyncClient.subscribe(subscription)
    ...
    override fun onDestroy() {
        ...
        App.pinpointManager.sessionClient.stopSession()
    }
}
